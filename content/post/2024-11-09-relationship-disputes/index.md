...
When I created the audio functionality in my angry robot debate, I was a little bit frustrated that I couldn't get my AI bots to just *seem* angry enough. That problem is now solved because OpenAI supports instructions for text to speech now. An updated implementation is below for those curious. The key changes are:

1. Created a Pydantic base class for the text output so that we can extract both a response and a tone of the response
2. Update the streaming output response to the new openAI standard. Remove pyaudio and instead use openai.helpers.LocalAudioPlayer
3. Make the stream function asynchrounous. Pass the tone as an argument to the function and the streaming output.
